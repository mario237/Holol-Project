        ...
        <div class="row align-items-center">
            ...
            <div class="col-7">
                <div class="text-left upgrade-btn">
                    <a href="{{url('dashboard/sliders/create')}}" class="btn btn-primary text-white">
                        ...
                        <div class="table-responsive table-hover mt-5">
                            <table class="table">
                                ...
                                @foreach($items as $item)
                                    <tr>
                                        <td>{{$item->id}}</td>
                                        <td><img src="{{url($item->image)}}" height="100px"/></td>
                                        <td>
                                            <div class="d-flex flex-row">
                                                <a class="btn btn-info mx-1"
                                                   href="{{url('dashboard/sliders/' . $item->id . '/edit')}}">
                                                    <i class="mdi mdi-account-edit "></i>
                                                </a>
                                                <form id="delete-form{{$item->id }}"
                                                      action="{{url('dashboard/sliders/' . $item->id)}}" method="POST">
                                                    @csrf()
                                                    @method('DELETE')
                                                </form>
