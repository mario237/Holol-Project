...
@section('main_content')
    ...
    <div class="page-breadcrumb">
        <div class="row align-items-center">
            <div class="col-5">
                <h4 class="page-title">طلبات العقارات</h4>
                <div class="d-flex align-items-center">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{url('dashboard')}}">اللوحة</a></li>
                            <li class="breadcrumb-item active" aria-current="page">طلبات العقارات</li>
                        </ol>
                    </nav>
                </div>
            </div>
    ...
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title">ادارة طلبات العقارات</h4>
                            ...
                            <table class="table">
                                ...
                                @foreach($list as $item)
                                    <tr class="{{($item->seen!='0')?'bg-light-gray':''}} ">
                                        ...
                                        <td>{{$item->phone}}</td>
                                        <td><a target="_blank"
                                               href="{{url('/services/estates/detail/'.$item->estate->id)}}">{{$item->estate->title}}</a>
                                        </td>
                                            ...
                                            <div class="d-flex flex-row">
                                                @if(in_array(\Illuminate\Support\Facades\Auth::user()->type,['1','4']) )
                                                    <form id="delete-form{{$item->id }}"
                                                          action="{{url('dashboard/orders/estates/' . $item->id)}}"
                                                          method="POST">
                                                        @csrf()
                                                        @method('DELETE')
                                                    </form>
                                                    <a class="btn btn-danger mx-1 text-white btn-sm"
