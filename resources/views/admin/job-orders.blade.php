...
@section('main_content')
    ...
    <div class="page-breadcrumb">
        <div class="row align-items-center">
            <div class="col-5">
                <h4 class="page-title">طلبات التوظيف</h4>
                <div class="d-flex align-items-center">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{url('dashboard')}}">اللوحة</a></li>
                            <li class="breadcrumb-item active" aria-current="page">طلبات التوظيف</li>
                        </ol>
                    </nav>
                </div>
            </div>
    ...
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title">ادارة طلبات التوظيف</h4>
                            ...
                            <table class="table">
                                ...
                                @foreach($list as $item)
                                    <tr class="{{($item->seen!='0')?'bg-light-gray':''}} ">
                                        ...
                                        <td>{{$item->fullname}}</td>
                                        <td>{{$item->email}}</td>
                                        <td><a href="{{url($item->cv_file)}}" target="_blank">افتح الملف</a></td>
                                        ...
                                        <td>
                                            <div class="d-flex flex-row">
                                                    ...
                                                    <form id="delete-form{{$item->id }}"
                                                          action="{{url('dashboard/orders/jobs/' . $item->id)}}"
                                                          method="POST">
                                                        @csrf()
                                                        @method('DELETE')
                                                    </form>
                                                    ...
                                                    @if($item->seen==0)
                                                        ...
                                                        <a onclick="$('#change-form{{$item->id}}').submit()"
                                                           class="btn btn-dark mx-1 text-white btn-sm">
                                                            تحديد كمشاهد
                                                        </a>
                                                    @else
